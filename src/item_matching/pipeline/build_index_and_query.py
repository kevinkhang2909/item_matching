...
class BuildIndexAndQuery:
    ...
    def build(self):
        ...
        if not path_index.exists():
            ...
            print(f'-> Building Index: {perf_counter() - start:,.2f}s')
        else:
            print(f'-> Index is existed')
    ...
    def query(self):
        ...
        print(f'-> Total batches: {num_batches} - Query size: {self.config.QUERY_SIZE:,.0f}')
